---
name: owly-code-standards
description: Enforces Owly project coding standards and I/O conventions, including naming, backend architecture, frontend boundaries, mobile specs, and timezones.
---
...
*   **No Local Time Storage:** Never add timezone offset (+7 hours) manually before saving raw strings to the database. This prevents errors inside PostgreSQL.
...
*   **Libraries:** Use libraries like `dayjs` or `date-fns`.

### 3. Frontend Presentation (Web & Mobile)
*   **Timezone Conversion:** The frontend receives ISO 8601 (UTC) dates from the API and converts them to local timezone.
*   **Formatting:** Use common helper functions to format dates for users.

---

## VI. Quality Automation

1.  **Auto-Formatting:** Use **Prettier**.
2.  **Linter:** Use **ESLint**.
...
- **Always dynamic:** Do not render a hardcoded "Lên lịch" badge for all `Scheduled` statuses. Check the date/time of the session dynamically against the current system time (`new Date()`).
- **Unified logic helper:** Keep this logic inside a helper or component property.
